module.exports = {
  route: 'lecturers',
  data: [
    {
      id: 'tm',
      data: {
        firstname: 'Tony',
        lastname: 'Maldonary',
        academic_title: 'B. Sc.',
        email: 'Tony.Maldonary@bachelor-of-science.com',
        salutation: 'Herr',
        phonenumber: '0123456789',
        mainFocus_ids: [
          '{{mainFocuses.projektmanagement.mainFocus_id}}',
          '{{mainFocuses.webentwicklung.mainFocus_id}}',
          '{{mainFocuses.recht.mainFocus_id}}',
          '{{mainFocuses.datenbanken.mainFocus_id}}',
        ],
        profile: 'My Profile',
        research: 'My Research',
        cv: 'My CV',
        comment: 'sehr engagiert',
        is_extern: false,
        allow_manipulation: false,
        possible_lectures: 'Recht, Datenbanken, Webentwicklung',
      },
      token: '{{users.gömert.token}}',
    },
    {
      id: 'eb',
      data: {
        firstname: 'Emmett L.',
        lastname: 'Brown',
        academic_title: 'Dr.',
        email: 'emmett.l@brown.edu',
        salutation: 'Mr.',
        phonenumber: '+1 1210 1985 2204',
        mainFocus_ids: [
          '{{mainFocuses.projektmanagement.mainFocus_id}}',
          '{{mainFocuses.softwareentwicklung.mainFocus_id}}',
        ],
        profile: '-',
        research: 'lots of stuff',
        cv: '-',
        comment:
          'can not hold lectures in the afternoon because he walks his dog from 3 to 5 pm; does not hesitate to cooperate with criminals',
        is_extern: true,
        allow_manipulation: true,
        possible_lectures: 'Projektmanagement',
      },
      token: '{{users.jreichwald.token}}',
    },
    {
      id: 'qq',
      data: {
        firstname: 'Quirinus',
        lastname: 'Quirrell',
        academic_title: '',
        email: 'quirrell@hogwarts.co.uk',
        salutation: 'Mr.',
        phonenumber: '+44 578 2108',
        mainFocus_ids: [],
        profile: '-',
        research: '',
        cv: '-',
        comment: 'Ravenclaw, hpmor.com',
        is_extern: true,
        allow_manipulation: true,
        possible_lectures: 'Verteidigung gegen die dunklen Künste',
      },
      token: '{{users.jreichwald.token}}',
    },
    {
      id: 'ea',
      data: {
        firstname: 'Elliot',
        lastname: 'Alderson',
        academic_title: null,
        email: 'elliotalderson@protonmail.ch',
        salutation: 'Herr',
        phonenumber: '(212) 555-0179',
        mainFocus_ids: ['{{mainFocuses.itsicherheit.mainFocus_id}}', '{{mainFocuses.mlaids.mainFocus_id}}'],
        profile: null,
        research: null,
        cv: 'Cybersecurity Engineer at Allsafe Cybersecurity',
        comment: 'paranoid',
        is_extern: true,
        allow_manipulation: false,
        possible_lectures: 'Neue Konzepte: IT Sicherheit, Grundlagen der WI',
      },
      token: '{{users.jreichwald.token}}',
    },
    {
      id: 'ps',
      data: {
        firstname: 'Patrick',
        lastname: 'Shyu',
        academic_title: null,
        email: 'exgoogleexfacebooktechlead@techlead.com',
        salutation: 'Herr',
        phonenumber: '(212) 555-0178',
        mainFocus_ids: ['{{mainFocuses.softwareentwicklung.mainFocus_id}}'],
        profile: null,
        research: null,
        cv: 'Ex-Google ex-Facebook techlead (as a millionaire)',
        comment: 'did we mention he\'s an ex-Google ex-Facebook techlead?"',
        is_extern: true,
        allow_manipulation: false,
        possible_lectures: 'Methoden und Werkzeuge des Software-Engineering, Software-Architekturen',
      },
      token: '{{users.matt.token}}',
    },
    {
      id: 'nc',
      data: {
        firstname: 'Noam',
        lastname: 'Chomsky',
        academic_title: null,
        email: 'noam.chomsky@uni-pensylvania.edu',
        salutation: 'Herr',
        phonenumber: '(212) 555-0179',
        mainFocus_ids: ['{{mainFocuses.softwareentwicklung.mainFocus_id}}'],
        profile: null,
        research: null,
        cv: '',
        comment: 'Chomsky hierarchy is named after him',
        is_extern: true,
        allow_manipulation: false,
        possible_lectures: 'Logik und Algebra, Mengenlehre',
      },
      token: '{{users.ritterbusch.token}}',
    },
    {
      id: 'es',
      data: {
        firstname: 'Edward',
        lastname: 'Snowden',
        academic_title: null,
        email: 'edward.snowden@gmail.rus',
        salutation: 'Herr',
        phonenumber: '(212) 555-0180',
        mainFocus_ids: ['{{mainFocuses.softwareentwicklung.mainFocus_id}}'],
        profile: null,
        research: null,
        cv: 'Worked For NSA and Open-Source',
        comment: 'Wanted in the USA under the Espionage Act of 1917',
        is_extern: true,
        allow_manipulation: false,
        possible_lectures: 'Software Engineering, Programmierung',
      },
      token: '{{users.gömert.token}}',
    },
    {
      id: 'bt',
      data: {
        firstname: 'Bakare',
        lastname: 'Tunde',
        academic_title: null,
        email: 'bakare.tunde@nigerian-prince.com',
        salutation: 'Herr',
        phonenumber: '(212) 555-0181',
        mainFocus_ids: ['{{mainFocuses.bwl.mainFocus_id}}', '{{mainFocuses.itsicherheit.mainFocus_id}}'],
        profile: null,
        research: null,
        cv: 'Nigerian Prince',
        comment: 'Needs money to return to his country',
        is_extern: true,
        allow_manipulation: false,
        possible_lectures: 'Finanzbuchhaltung, Grundlagen der BWL,Neue Konzepte: IT-Sicherheit',
      },
      token: '{{users.gömert.token}}',
    },
    {
      id: 'tr',
      data: {
        firstname: 'Taschen',
        lastname: 'Rechner',
        academic_title: null,
        email: 'Taschen-Rechner@gmail.com',
        salutation: 'Herr',
        phonenumber: '(212) 555-0182',
        mainFocus_ids: ['{{mainFocuses.m.mainFocus_id}}'],
        profile: null,
        research: null,
        cv: 'Good at maths',
        comment: 'Can calculate a lot in his head',
        is_extern: true,
        allow_manipulation: false,
        possible_lectures: 'Statistik, Logik und Algebra, Operations Research',
      },
      token: '{{users.gömert.token}}',
    },
  ],
};
